...
static VertexInfo all_vertices[MAX_VERTICES_NUM];
static size_t all_vertices_size = 0;
...
VertexInfo* add_vertex(VertexInfo* einfo)
{
	...
	if(all_vertices_size < MAX_VERTICES_NUM){
		memcpy(&all_vertices[all_vertices_size], einfo, sizeof(all_vertices[0]));
...
void remove_all_vertices()
{
	for(size_t i = 0; i < all_vertices_size; i++)
	{
		...
		memset(&all_vertices[i], 0, sizeof(all_vertices[0]));
	}
	all_vertices_size = 0;
}
